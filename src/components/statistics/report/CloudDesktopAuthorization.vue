                    ...
                    el-card(:body-style="{padding: '16px 16px 0 16px'}")
                        el-form(inline size="small" label-width="80px")
                            ...
                            el-form-item(label="云桌面")
                                el-select(v-model="searchParams.desktop_group_id" placeholder="请选择云桌面" clearable)
                                    el-option(v-for="item in desktopList" :key="item.delivery_group_id" :value="item.delivery_group_id" :label="item.published_name")
...
<script>
     ...
     */
    export default {
        name: 'CloudDesktopAuthorization',
        mixins: [reportMixin],
        ...
        methods: {
            ...
            getReport () {
                ...
                Api.get('user_desktops', paramsWithPage, true).then(res => {
                    // 搜索的添加到导出过滤项
            ...
            exportReport () {
                Api.get('user_desktops_export', this.exportParams, true, 'blob').then(res => {
            ...
            getDesktopList () {
                Api.get('desktop_list', {}, true).then(res => {
                    this.desktopList = res
                })
            }
        },
        created () {
