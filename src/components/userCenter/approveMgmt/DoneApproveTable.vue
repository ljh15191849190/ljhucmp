<template lang="pug">
    UcmpTableContainer(
        ...
        div.full-height(slot="tableContainer")
            //- bug UCMP-746 已审批申请单列表加载中，请加loading（云首图标），让用户知道当前正在加载中
            el-table(:data="tableData" v-loading="isLoading" element-loading-spinner="ucmp-icon-loading" border)
                el-table-column(
                    ...
                    template(slot-scope="scope")
                        //- router-link(:to="`/orders/${scope.row.order_code}`" v-if="column.prop === 'order_code'" @click="setActivedNavIndex('/orders')") {{scope.row.order_code}}
                ...
                el-table-column(prop="operation" key="operation" label="操作")
                    template(slot-scope="scope")
                        IconButton(
                            type="ucmp-icon-view"
                            text="查看审批详情"
                            @iconClick="openTrackDialog(scope.row, true)")
...
export default {
    ...
    methods: {
            ...
            Api.get('approveList', params, true).then(
                res => {
                    ...
                    if (res && Array.isArray(res.list)) {
                        ...
                        this.pagination.count = res.size
                    }
        ...
        openTrackDialog (approve, isViewDetail = false) {
            if (isViewDetail) {
                let queryParams = {
